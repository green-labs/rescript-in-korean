...
title: 'Object'
metaTitle: 'Object'
metaDescription: 'Interoping with JS objects in ReScript'
sourceUrl: 'https://rescript-lang.org/docs/manual/latest/object'
---


리스크립트의 객체는 [레코드]()와 비슷합니다. 하지만...
...
노트: 위에서 언급했다시피 레코드와 다르게 여기서 `me`는 `"age"`, `"name"` 필드가 있는 타입 선언을 찾아서 비교하려하지 **않습니다**.
...
이는 타입체커가 `me`를 `person`과 매칭하려 시도하지 않기 때문입니다.
...
이 기법이 매우 훌륭한 이유는 사용하고자 하는 JS 라이브러리의 바인딩이 있든 없든 전혀 신경쓰지 않고도 일단 리스크립트 코드를 작성할 수 있기 때문입니다.
