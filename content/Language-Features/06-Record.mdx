...
레코드(Record)는 JavaScript 객체(Object)와 유사합니다. 다만,
...
### JavaScript 출력
ReScript 레코드는 JavaScript 객체로 컴파일 됩니다. 
...
getAge(me) // 타입 오류!
```
타입 시스템은 `me` 가 `person` 이고, `getAge` 는 `monster` 타입으로만 작동한다고 할 것입니다. 이러한 기능이 필요하다면 ReScript 객체(object) 를 사용해보세요.
