...
하지만 위 배열 예제는 **매우 권장하지 않습니다.** (튜플을 사용하세요) 또한 목록 예제는 오류가 발생합니다.
각 예제는 형식만 완전합니다.
...
패턴 매칭의 구조분해 측면은 좋습니다만, 실제로 코드 구조를 생각하는 방식이 변경되지는 않습니다.
생각하는 패러다임을 바꾸는 방법은 데이터 형태를 보고 코드를 실행하는 것입니다.
...
다른 언어에서는 읽기 어렵고 오류가 발생하기 쉬운 if-eles 조합으로 작성합니다.
대신 리스크립트는 강화 된 `switch` 패턴 매칭 기능을 사용해 구조분해 기반으로 올바른 코드를 호출하면서 값을 구조분해 할 수 있습니다.
...
갑자기 값을 지저분하게 확인하는 if-eles 조합이 정확한 값의 형태를 기반으로 실행할 수 있는 깔끔하고 컴파일러가 검증한 선형적인 코드로 변경됐습니다.

### 복잡한 예시

Here's a real-world scenario that'd be a headache to code in other languages. Given this data structure:

다른 언어로 코딩할 때 골치아픈 시나리오 예시를 보여드리겠습니다.
...
훨씬 더 선형적이네요! 이제 코드조각을 이렇게 작성 할 수도 있겠죠?
