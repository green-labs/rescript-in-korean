...
```
선언에서 `~x` 부분은 함수가 `x`로 표시된 인수를 받아들이고 동일한 이름으로 함수에서 참조 할 수 있음을 의미합니다.
간결하게 작성하기 위해 함수 내부에서 다른 이름으로 인수를 참조 할 수도 있습니다.
...
```
사실 `(~radius)`는 `(~radius as radius)`의 축약입니다.

인수에 타입을 기재하는 문법은 다음과 같습니다.
...
**참고** 타입 시스템을 위해 선택적 인수가 있을때마다 그 뒤에 위치 인수(라벨이 지정되지 않았고 선택적 인수가 아닌 인수)가 하나 이상 있는지 확인해야합니다.
없는 경우 더미 `unit` (일명 `()`) 인수를 기재합니다.
_(역주: 위 코드 조각에서 `~radius=?, ()` 부분에 해당하는 설명입니다.)_

### 시그니쳐 및 타입 어노테이션

선택적 레이블 인수가 있는 함수는 시그니쳐 및 타입 어노테이션과 관련해 혼란스러울 수 있습니다.
실제로 선택적 레이블이 지정된 인수의 타입은 함수를 호출하는지, 함수 내부에서 작업하는지에 따라 다르게 보입니다.
함수 외부에서 원시 값을 전달하거나 (예:`int`) 전달하지 않을 수 있습니다.
함수 내부에서는 매개 변수가 항상 있지만 그 값은 option 입니다 (`option <int>`).
...
레이블이 지정된 선택적 인수를 사용해 함수 시그니처를 작성할 때 어려움을 겪는 경우 위 내용을 기억하세요!
...
ReScript는 기본적으로 함수내에서 재귀적으로 호출하는 것을 방지하고 있습니다.
...
ReScript의 함수는 기본적으로 커리드 함수며, 컴파일 된 JS 결과물에서 우리가 지불하는 몇 가지 성능 패널티 중 하나입니다.
