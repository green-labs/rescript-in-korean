...
metaDescription: 'ReScript에서 nullable optional 값과 JS 인터롭(상호운용)'
---

ReScript 자체는 `null` 또는 `undefined`에 대한 개념이 없습니다. 이건 버그의 전체 범주를 제거하는 정말 _훌륭한_ 일입니다. 더 이상의 `undefined is not a function`, 그리고 `cannot access someAttribute of undefined` 에러는 없습니다!

그러나 잠재적으로 존재하지 않는 값의 **개념**은 여전히 유용하며, ReScript에서도 안전하게 존재합니다.

ReScript에서는 `option`타입으로 값을 감싸서 존재하는 값과 존재하지 않는 값을 표현합니다. `option`타입의 표준 라이브러러리 정의는 다음과 같습니다:
...
**Note** `option` 타입은 단지 일반적인 [variant](08-Variant.mdx) 입니다.
...
이제 licenseNumber는 option 타입입니다. 나중에 다른 코드가 이 값을 받으면 [패턴 매칭](15-Pattern-Matching-Destructuring.mdx)을 통해 `Some('a)`와 `None`두 값을 모두 처리해야합니다.
...
위 예제에서는 일반적인 숫자를 `option` 타입으로 바꾸고 `None` 케이스에 대한 처리를 강제함으로써 ReScript는 개념적인 `null`값을 잘못 처리하거나 처리를 깜빡 잊을 가능성을 효과적으로 제거했습니다! **순수 ReScript 프로그램에는 null 에러가 없습니다**.

## JavaScript undefined와 null의 Interoperate
...
안타깝게도 많은 경우에 JavaScript 값은 `null` 또는 `undefined` *양쪽*일 수 있습니다._(역주. 양쪽 다 존재하지 않는 값의 개념이지만 JavaScript에서는 null과 undefined가 다른 취급을 받습니다.)_
...
ReScript 쪽에서 nullable 문자열을 생성하려면(아마 인터롭을 목적으로 JS 쪽에 전달하는 경우입니다) 다음처럼 하세요.
