...
왜 싱글 페이로드를 가진 배리언트 또는 레코드가 필요할까요? 왜 그냥 페이로드를 넘기지 않을까요? 여기에 배리언트의 사용 예가 있습니다.

지역 및 전역 좌표 시스템을 가진 게임이 있다고 가정해봅시다. 코드는 다음과 같습니다.
...
이런, 뭔가 잘못되었습니다. `renderDot` 은 지역좌표가 아니라 전역 좌표를 가져야 합니다. 좌표의 종류를 넘겨서 해결해봅시다.
...
이제 `renderDot`은 `worldCoordinates`만을 입력으로 받습니다. `unboxed`속성과 분리된 배리언트 타입과 인자값의 분해를 조합하여 성능의 손해가 없고 안전하게 깔끔하며 배리언트 래퍼가 적은 자바스크립트 코드로 변환되었습니다. 출력을 점검해보세요.