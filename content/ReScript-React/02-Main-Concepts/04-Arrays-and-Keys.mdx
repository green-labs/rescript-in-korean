...
데이터를 엘레멘트 배열로 변환하고 이를 리액트 트리에 넣을때마다 리액트가 렌더링에 대해 각 엘레멘트마다 구별할 수 있도록 고유한 식별자를 넣어야합니다. 이 페이지에서는 `key`속성과 데이터를 `React.element` 타입에 매핑할 때마다 적용하는 방법을 설명합니다.

## 키 그리고 배열 렌더링하기

키는 리액트가 각 렌더링 과정에서 변경, 추가 또는 제거 된 엘레멘트를 식별하는 데 도움이 됩니다. 엘레멘트에 안정적인 ID를 제공하려면 배열 내부 엘레멘트에 키를 주입해야합니다.
