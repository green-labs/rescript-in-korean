...
분명히 `timeId`는 `setTimeout`에 의해서만 만들어질 수 있는 타입입니다! 이제 `clearTimeout`이 반드시 유효한 ID를 전달할 것을 명확하게 해봅시다. 이걸 내부적으로 쓸지 아닐지는 구현하기에 따라 달려있습니다
...
`__filename` , `__DEV__` 와 같은 전역변수는 항상 존재하진 않습니다. 심지어 이러한 전역변수들은 `option`으로 모델링할 수 없습니다. 리스크립트에서는 그들을 참조하는 작은 행위도 브라우저 환경에서 보통 `Uncaught ReferenceError: __filename is not defined` 와 같은 오류를 발생할 수 있기때문입니다. 


이러한 전역변수의 문제들을 해결하기위해 리스크립트는 `%external(a_single_identifier)` 와 같은 접근을 제공하고 있습니다.
...
첫번째줄의 `typeof` 와 같은 확인은 자바스크립트의 참조오류가 발생되는 것을 막을 수 있습니다.
